...
SOURCE_IMAGE_DIR = './data/icg_drone/dataset/resize_original'
SOURCE_LABEL_DIR = './data/icg_drone/dataset/resize_label'
...
for source_image in tqdm.tqdm(source_image_paths[:240]):
    ...
    image_file_index.append('{:s} {:s}'.format(SOURCE_IMAGE_DIR+'/'+source_image, label_image_path))
...
for source_image in tqdm.tqdm(source_image_paths[240:320]):
    image_name = ops.split(source_image)[1]
    image_id = image_name.split('.')[0]
    label_image_name = '{:s}.png'.format(image_id)
    label_image_path = ops.join(SOURCE_LABEL_DIR, label_image_name)
